...
class RestauranteApp(ctk.CTk):
    def __init__(self):
        ...
        self.ingredients = {}  # Diccionario para almacenar los ingredientes

        # Crear pestañas
        self.tabview = ctk.CTkTabview(self)
        self.tabview.pack(expand=1, fill="both")

        # Pestañas de ingredientes y pedidos
    ...
    def create_ingredients_tab(self):
        # Etiqueta y campo de entrada para el nombre del ingrediente
        name_label = ctk.CTkLabel(self.tab_ingredientes, text="Nombre del Ingrediente:")
        name_label.pack(pady=(20, 5))
        self.ingredient_name_entry = ctk.CTkEntry(self.tab_ingredientes, placeholder_text="Nombre del Ingrediente")
        self.ingredient_name_entry.pack(pady=(0, 10))

        # Etiqueta y campo de entrada para la cantidad del ingrediente
        qty_label = ctk.CTkLabel(self.tab_ingredientes, text="Cantidad:")
        qty_label.pack(pady=(10, 5))
        self.ingredient_qty_entry = ctk.CTkEntry(self.tab_ingredientes, placeholder_text="Cantidad")
        self.ingredient_qty_entry.pack(pady=(0, 10))

        # Botones para agregar, eliminar ingredientes y generar el menú
        ...
        # Listbox para mostrar los ingredientes
        self.ingredient_listbox = Listbox(self.tab_ingredientes, selectmode="single", height=8)
        self.ingredient_listbox.pack(expand=0, fill="both", pady=10)
    ...
    def create_orders_tab(self):
        # Etiqueta inicial para gestión de pedidos
        ctk.CTkLabel(self.tab_pedidos, text="Gestión de Pedidos").pack(pady=10)

        # Iconos o imágenes para los pedidos (se pueden ajustar o cambiar)
        icon = PhotoImage(file="image/icono_cola_64x64.png")  # Cambiar la ruta al icono real
        icon_label = ctk.CTkLabel(self.tab_pedidos, image=icon)
        icon_label.image = icon  # Mantener referencia para evitar que la imagen sea eliminada
        icon_label.pack(pady=10)

        # Otros componentes para gestionar pedidos se pueden agregar aquí
